...
// 绑定所有事件（核心：优化触摸事件，阻止默认行为，解决手机下滑问题）
function bindAllEvents() {
    ...
    // 核心优化：使用addEventListener绑定触摸事件，明确passive: false，确保preventDefault生效
    // 移除旧的ontouchstart/ontouchend绑定，避免冲突
    document.addEventListener('touchstart', handleTouchStart, { passive: false });
    document.addEventListener('touchend', handleTouchEnd, { passive: false });
    // 额外绑定touchmove事件，阻止页面滚动（双重保障）
    document.addEventListener('touchmove', function(e) {
        e.preventDefault(); // 彻底阻止触摸移动导致的页面滚动
    }, { passive: false });
}
...
// 触摸开始（核心：阻止默认行为，解决手机下滑页面滚动问题）
let touchX = 0, touchY = 0;
function handleTouchStart(e) {
    if (isGameOver) return;
    e.preventDefault(); // 关键1：阻止浏览器默认触摸行为（页面滚动、拖动）
    touchX = e.touches[0].clientX;
    touchY = e.touches[0].clientY;
}

// 触摸结束（核心：阻止默认行为，提高滑动阈值，确保瓦片响应）
function handleTouchEnd(e) {
    if (isGameOver) return;
    e.preventDefault(); // 关键2：阻止浏览器默认触摸行为
    const endX = e.changedTouches[0].clientX;
    const endY = e.changedTouches[0].clientY;
    const diffX = endX - touchX;
    const diffY = endY - touchY;
    const slideThreshold = 30; // 提高阈值到30，降低误触，提升响应优先级
