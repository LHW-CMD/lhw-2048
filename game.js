...
function initDOM() {
    ...
    dom.highScoreEl = document.getElementById('highScore') || null; // 新增
    dom.bestTimeEl = document.getElementById('bestTime') || null;   // 新增
}
...
// 注册功能（新增：初始化用户最高成绩和用时）
function handleRegister() {
    if (!dom.usernameInput || !dom.passwordInput) return;
    ...
    const userList = JSON.parse(localStorage.getItem('2048UserList')) || [];
...
function handleLogin() {
    if (!dom.usernameInput || !dom.passwordInput || !dom.gamePanel || !dom.loginPanel) return;
    const username = dom.usernameInput.value.trim();
    const password = dom.passwordInput.value.trim();
...
function checkLoginStatus() {
    if (!dom.gamePanel || !dom.loginPanel) return;
    const currentUser = localStorage.getItem('2048CurrentUser');
    if (currentUser) {
        dom.loginPanel.style.display = 'none';
        dom.gamePanel.style.display = 'block';
        // 初始化最高成绩和最佳用时显示
